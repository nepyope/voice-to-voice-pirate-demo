...
app_port: 7860
pinned: false
short_description: Realtime voice over WebSocket or WebRTC
hf_oauth: true
...
- **`LOAD_BALANCER_URL` env** — multi-compute deployments only: the browser
  ...
  request a session or join the queue.
...
| `REQUIRE_LOGIN` | _(unset)_ | Require Hugging Face sign-in before session allocation |
