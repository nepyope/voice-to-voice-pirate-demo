...
search, OAuth and load-balancer helpers remain available, but Compose selects
the fixed backend and leaves metering and search disabled.
"""
...
from realtime_proxy import relay, backend_ready, pool_url
...
with open(os.path.join(HERE, "pirate.json"), encoding="utf-8") as f:
    PIRATE = json.load(f)
if SPEECH_TO_SPEECH_URL:
...
async def realtime(websocket: WebSocket):
    await relay(websocket, SPEECH_TO_SPEECH_URL)


@app.get("/api/health")
async def health():
    ready = await backend_ready(SPEECH_TO_SPEECH_URL)
    sdk = any(os.path.isfile(p) for p in (
        os.path.join(HERE, "vendor/openai-realtime-agents.umd.js"),
        os.path.join(HERE, "node_modules/@openai/agents-realtime/dist/bundle/openai-realtime-agents.umd.js"),
    ))
    return JSONResponse({"ready": ready and sdk, "backend": ready, "sdk": sdk},
                        status_code=200 if ready and sdk else 503)
...
def agents_sdk_bundle():
    ...
    if not os.path.isfile(path):
        raise HTTPException(status_code=503, detail="Run npm ci in the Space repository.")
...
def config():
    ...
    return {
        ...
        # Browser-visible same-origin relay; the target remains server-side.
        "s2sUrl": "/api/realtime" if SPEECH_TO_SPEECH_URL else "",
        "instructions": PIRATE["instructions"],
        "voice": PIRATE["voice"],
        # This package validates the WebSocket path. WebRTC is not offered.
        "rtc": False,
...
# Register static assets last so API routes take precedence.
class DemoStaticFiles(StaticFiles):
    async def get_response(self, path, scope):
        # Serve the UI, never Python source, local env files or npm metadata.
        top = path.split("/", 1)[0]
        allowed = {".", "", "index.html", "main.js", "style.css", "s2s-realtime-client.js", "pirate.json"}
        if path not in allowed and top not in {"ui", "ws", "worklets"}:
            raise HTTPException(status_code=404)
        if top in {"ui", "ws", "worklets"} and not path.endswith(".js"):
            raise HTTPException(status_code=404)
