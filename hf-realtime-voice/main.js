...
import { S2sRealtimeClient } from "./s2s-realtime-client.js?v=audio-24k-20260901-1";
...
const STORAGE_KEYS = {
  ...
  directUrl: "pirate.v1.ws.directUrl",
  voice: "pirate.v1.ws.voice",
  instructions: "pirate.v1.ws.instructions",
  tools: "pirate.v1.ws.tools",
  searchKey: "pirate.v1.ws.searchKey",
  noiseGate: "pirate.v1.ws.noiseGate",
  // "ws" | "webrtc". Not under the historical "pirate.v1.ws." prefix — it selects
  // between the transports rather than configuring the WS one.
  transport: "pirate.v1.transport",
  audioInputId: "pirate.v1.audio.inputId",
  audioOutputId: "pirate.v1.audio.outputId",
};
...
function loadTools() {
  try {
    ...
    return { web_search: false, camera_snapshot: false };
  }
}
...
async function fetchConfig() {
  try {
    ...
    if (res.ok) {
      ...
      if (!localStorage.getItem(STORAGE_KEYS.instructions)) settings.instructions = json.instructions || DEFAULT_INSTRUCTIONS;
...
function syncTransportUi() {
  ...
  transportField.hidden = !rtcAvailable;
...
async function doStart(audioContext = null) {
  if (!configReady) throw new Error("Demo configuration unavailable. Start the UI server and reload.");
  if (!window.isSecureContext) throw new Error("Microphone access needs HTTPS or localhost. See the README for remote access.");
